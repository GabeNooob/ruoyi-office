...
public class OaLocalNotificationListener implements ApplicationListener<BpmProcessInstanceStatusEvent> {
    ...
    @Override
    public void onApplicationEvent(@NotNull BpmProcessInstanceStatusEvent event) {
        log.info("[onApplicationEvent][本地事件] 收到BPM事件: {}", event);

        try {
            // 参数校验
            if (event.getProcessInstanceInfo() == null || event.getEventType() == null) {
                log.warn("[handleBpmMessage] 流程实例信息或事件类型为空");
                return;
            }

            String processDefinitionKey = event.getProcessInstanceInfo().getProcessDefinitionKey();
            String businessKey = event.getProcessInstanceInfo().getBusinessKey();
            BpmEventTypeEnum eventType = event.getEventType();

            // 参数校验
            if (StrUtil.isBlank(processDefinitionKey) || StrUtil.isBlank(businessKey)) {
                log.warn("[handleBpmMessage] 参数不完整，processDefinitionKey: {}, businessKey: {}, eventType: {}",
                        processDefinitionKey, businessKey, eventType);
                return;
            }

            // 只处理OA相关流程
            if (!processDefinitionKey.startsWith("oa_")) {
                log.debug("[handleBpmMessage] 非OA流程，跳过处理: {}", processDefinitionKey);
                return;
            }

            log.info("[handleBpmMessage] 处理OA BPM事件，eventType: {}, processDefinitionKey: {}, businessKey: {}",
                    eventType.getName(), processDefinitionKey, businessKey);
            ...
            log.error("[onApplicationEvent][本地事件] 处理BPM事件失败", e);
        }
    }
